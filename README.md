...
**KOICA 규정 38개를 7개 MCP 도구로.** 한국국제협력단 내부 규정·관련 법령을 조문 단위 검색 + 본문 직접 조회 + 인용 검증 + 상호참조 그래프로, Claude Desktop·Cursor·Windsurf 등 MCP 호환 클라이언트에서 바로 사용.
...
Claude Desktop을 **완전 종료(Cmd+Q)** 후 재실행하면 입력창 하단 도구 메뉴에 `koica-reg` 7개 도구가 보입니다.
...
- [ ] v1.3 — 음절 bi-gram 매칭 (형태소 변종 대응, "사례금" ↔ "사례비" 등)
