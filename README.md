...
**KOICA 규정 38개를 6개 MCP 도구로.** 한국국제협력단 내부 규정·관련 법령을 조문 단위 검색 + 본문 직접 조회 + 인용 검증 + 상호참조 그래프로, Claude Desktop·Cursor·Windsurf 등 MCP 호환 클라이언트에서 바로 사용.
...
Claude Desktop을 **완전 종료(Cmd+Q)** 후 재실행하면 입력창 하단 도구 메뉴에 `koica-reg` 6개 도구가 보입니다.
...
| 도구 | 입력 | 반환 |
|---|---|---|
| `search_regulation` | `query, category?, source?, limit?` | 조문 단위 검색 결과 (snippet, score, citation 포함) |
...
평문 PDF 추출본(`제1조(목적) 본문 …` 형식)도 파서가 자동 처리합니다.

---

## 규정 갱신·추가 요청

KOICA 규정은 정기적으로 개정됩니다. 새 개정판이 나오거나 누락된 규정 추가가 필요하면 **Issues로 요청**해 주세요. 메인테이너가 추출·검증·커밋한 후 알려드립니다.

- [규정 갱신·추가 요청](../../issues/new?template=regulation-update.md)
- [버그·정확도 신고](../../issues/new?template=bug-report.md)

요청에 PDF를 첨부하시면 처리 시간이 단축됩니다.
...
검색 정확도 (questions.json 기반 자동 평가, n=150):
- top-1: ~31%
- top-3: ~60%
- top-10: ~85%
...
- [x] v1.1 — source 토큰 매칭 fallback (한국어 조사·연결어에 관대) + 본문 메타 태그(`<개정 …>`, `[제목개정 …]`) 정리
- [ ] v1.2 — `external_law_citations` 필드: KOICA 규정 안의 외부 법령 인용을 자동 추출해 korean-law-mcp 연계 강화
- [ ] v1.3 — PDF→md 자동 변환 스크립트
- [ ] v1.4 — 음절 bi-gram 매칭 (형태소 변종 대응)
- [ ] v1.5 — 별표·별지 분리 인덱싱 (행정처분 기준표, 서식 등)
