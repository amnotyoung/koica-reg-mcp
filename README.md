...
> `verify_citation`은 한 번에 텍스트 20,000자·인용 200건까지 검증합니다. 이를 넘으면 앞부분만 검증하고 마지막에 `status`가 `truncated`인 항목으로 미검증 건수를 알려 주니, 긴 문서는 나눠서 요청하세요.
...
통과한 PR은 squash 병합되고 Fly.io 재배포까지 이어지므로 사람의 정기 확인이
필요하지 않습니다.
...
> ⚠️ 본 데이터는 참조·업무 보조용이며 정확성을 보증하지 않습니다. **법적 효력이 있는 공식·최신 규정본은 반드시 [ALIO](https://www.alio.go.kr) 또는 KOICA 공식 채널에서 확인**하세요.
